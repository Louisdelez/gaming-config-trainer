...
export default function Tracking() {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      // Update target with bouncing + drift
      if (Math.random() < 0.02) {
      ...
      if (holding.current) {
        const dx = cursorPos.current.x - targetPos.current.x;
        const dy = cursorPos.current.y - targetPos.current.y;
  ...
  }, [running, canvasSize.w, canvasSize.h]);
...
<div className="bg-[#1f1f1f] px-4 py-3 flex items-center justify-between border-b border-white/[0.04]">
      ...
      <div ref={canvasRef} className="game-canvas">
        <AimZone
          ...
          onMove={(x, y) => { cursorPos.current = { x, y }; }}
        ...
        >
          ...
          {!running && accuracyPct == null && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 text-center px-6 pointer-events-none">
