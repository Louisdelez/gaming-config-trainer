...
export default function Flickshot() {
  ...
  const spawnTarget = () => {
    ...
    // Random angle, random distance 150-300px from center
    const angle = Math.random() * Math.PI * 2;
    const dist = 150 + Math.random() * 200;
    const x = Math.max(TARGET_SIZE, Math.min(rect.width - TARGET_SIZE, cx + Math.cos(angle) * dist));
    const y = Math.max(TARGET_SIZE, Math.min(rect.height - TARGET_SIZE, cy + Math.sin(angle) * dist));
    setTarget({ x, y });
  ...
  const start = () => {
    ...
    setShowCenter(true);
    setTarget(null);
    // Initial small delay before first target
  ...
  const handleTargetHit = () => {
    ...
    if (newTimes.length >= TOTAL_SHOTS) {
      ...
      // Wait for player to return to center area before next spawn
  ...
  return (
    <GameShell
      icon={Zap}
      title={t("nav.flickshot")}
      description={t("games.flickshotDesc")}
      gameKey="flickshot"
      unit="ms"
      currentScore={avgMs}
    ...
    >
      ...
      <div ref={canvasRef} className="game-canvas relative h-[480px] bg-[#121212]">
        {showCenter && (
        ...
        {target && (
          <button
            ...
            className="absolute rounded-full bg-[#1ed760] shadow-[0_0_20px_rgba(30,215,96,0.5)]"
            style={{ left: target.x - TARGET_SIZE / 2, top: target.y - TARGET_SIZE / 2, width: TARGET_SIZE, height: TARGET_SIZE }}
          />
        )}
        {!running && shotTimes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center text-slate-400">{t("games.clickToStart")}</div>
        )}
